...
class GradedForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.exercise.get('show_correct_once', False):
    ...
    def add_field(self, i, config, field_class, widget_class,
        ...
        if random_attributes:
            config['random_sample'] = random_attributes['sample']
    ...
    def grade_checkbox(self, configuration, value, hints=None):
        ...
        wrong_answers = 0
        non_neutral_count = 0
        randomized_sample = configuration.get('random_sample')
        if randomized_sample:
            sample = [int(x) for x in randomized_sample.split('-')]
        else:
            sample = range(len(configuration.get("options", [])))
